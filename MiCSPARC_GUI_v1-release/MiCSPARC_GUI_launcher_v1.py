...
class CreatePFNReferencesTab(QWidget):
    def __init__(self):
        ...
        self.legacy_button = QPushButton("Run Legacy mode Create PFN References")
        self.threads_input = QSpinBox()
        ...
        layout.addWidget(QLabel("Threads: only for legacy mode"))
        layout.addWidget(self.threads_input)
        self.setLayout(layout)
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.script_dir_button = QPushButton("Browse")
        ...
        script_layout.addWidget(QLabel("Path to MiCSPARC scripts directory:"))
        ...
        self.createpfn_tab.legacy_button.clicked.connect(self.run_createpfn_legacy_script)
    ...
    def run_createpfn_legacy_script(self):
        cmd = [
    ...
    def run_createpfn_script(self):
        cmd = [
            "python", self.build_script_path("csparc_create_pfn_references_v3_GUI.py"),
            "--i", os.path.basename(self.createpfn_tab.input_file.text()),
            "--recenter", self.createpfn_tab.recenter.text(),
            "--apix", self.createpfn_tab.apix.text(),
            "--pfn", self.createpfn_tab.pfn.text()
    ...
    def show_createpfn_help(self):
        self.run_script(["python", self.build_script_path("csparc_create_pfn_references_v3_GUI.py"), "--help"], cwd=self.output_dir)
